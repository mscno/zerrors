...
func (m *LocalizedMessage) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.LocalizedMessage) > 0 {
		i -= len(m.LocalizedMessage)
		copy(dAtA[i:], m.LocalizedMessage)
		i = encodeVarint(dAtA, i, uint64(len(m.LocalizedMessage)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.Key) > 0 {
...
func (m *LocalizedMessage) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			m.LocalizedMessage = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skip(dAtA[iNdEx:])
